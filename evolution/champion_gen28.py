"""
Orbit Wars V49 — The Two-Faced Goliath (Frankenstein C+).

Evolution from V39 (gold baseline, 729.9 ELO).
Key change: True Dual-Brain Context Switching.
Plays exactly like V39 in FFA mode (aggressive cross-map snipes), but
when the game is a Duel (or an FFA reduces to 2 players), it activates
V48's Soft Locality Bonus and V36's Dynamic Profitability to play safe
and efficiently consolidate local borders.
...
LOCALITY_RADIUS = 21.8    # V43: Max distance for locality bonus
LOCALITY_BONUS_MAX = 12.3 # V43: Max additive bonus for immediate neighbors
...
class Context:
    ...
    def _build_power_table(self):
        ...
        self.duel_opening = self.is_duel and w.step < 61
        ...
        self.ffa_opening = (not self.is_duel) and w.step < 45
        self.ffa_behind = (
    ...
    def _reserve_for(self, planet, nearest_enemy):
        ...
        if self.ffa_behind and incoming <= 0 and nearest_enemy >= 34.8:
            return max(3, planet.production + 1)
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            base = t.production * (92.0 if t.owner == -1 else 135.2)
            if self.duel_opening and t.owner == -1:
                base += t.production * 73.3
                base += max(0.0, 20.6 - t.ships) * 2.2
            if self.ffa_opening and t.owner == -1:
                base += t.production * 26.4
                base += max(0.0, 17.7 - t.ships) * 0.9
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        # V49 (Blueprint C+): Dynamic profitability in ANY duel scenario.
        # Switched from _init_nplayers to ctx.is_duel so that if an FFA
        # becomes a 2-player game, we flip the switch to Duel logic.
        if ctx.is_duel:
        ...
        # V49 (Blueprint C+): Dual-Brain Locality.
        # Only active when it's a duel (or an FFA that has become a duel).
        if ctx.is_duel and distance < LOCALITY_RADIUS:
            base += LOCALITY_BONUS_MAX * (1.0 - distance / LOCALITY_RADIUS)

        if ctx.duel_opening and target.owner == -1:
            score = base - send * 1.15 - distance * 1.16 - eta * 1.76
            ...
            score = base - send * 1.83 - distance * 0.83 - eta * 1.08
            if target.production >= 3:
                score += 17.0
            if target.ships <= 14:
                score += 14.6
            ...
            score = base - send * 2.21 - distance * 0.71 - eta * 1.20
