...
LOCALITY_RADIUS = 30.7    # V43: Max distance for locality bonus
...
class Context:
    ...
    def _reserve_for(self, planet, nearest_enemy):
        ...
        if self.ffa_behind and incoming <= 0 and nearest_enemy >= 36.6:
            return max(3, planet.production + 1)
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        if ctx.duel_opening and target.owner == -1:
            ...
            score = base - send * 1.88 - distance * 0.89 - eta * 1.08
            if target.production >= 3:
                score += 16.1
            if target.ships <= 14:
                score += 14.0
            ...
            score = base - send * 2.29 - distance * 0.56 - eta * 1.13
