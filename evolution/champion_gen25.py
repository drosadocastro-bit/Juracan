...
class Context:
    ...
    def _build_power_table(self):
        ...
        self.duel_opening = self.is_duel and w.step < 55
        ...
        self.ffa_opening = (not self.is_duel) and w.step < 47
        self.ffa_behind = (
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            base = t.production * (92.0 if t.owner == -1 else 122.8)
            if self.duel_opening and t.owner == -1:
                base += t.production * 68.5
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        if ctx.duel_opening and target.owner == -1:
            score = base - send * 1.24 - distance * 1.25 - eta * 1.76
            ...
            score = base - send * 1.88 - distance * 0.89 - eta * 1.04
            if target.production >= 3:
                score += 16.1
            if target.ships <= 14:
                score += 14.0
            ...
            score = base - send * 2.29 - distance * 0.60 - eta * 1.13
