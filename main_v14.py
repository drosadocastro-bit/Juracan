...
class Context:
    ...
    def _build_power_table(self):
        ...
        for p in w.planets:
            if p.owner >= 0:
                power[p.owner] += p.ships + p.production * 17.315598855459246
        for f in w.fleets:
            if f.owner >= 0:
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            base = t.production * (114.23306750710755 if t.owner == -1 else 98.05194571248872)
            if self.duel_opening and t.owner == -1:
                base += t.production * 98.44843968142928
                base += max(0.0, 18.0 - t.ships) * 3.569859117549806
            if self.ffa_opening and t.owner == -1:
                base += t.production * 14.726567931476886
                base += max(0.0, 18.0 - t.ships) * 0.688886135813734
                if t.production >= 3 and t.ships <= 16:
                    base += 26.31689599956083
            if self.ffa_behind and t.owner == self.leader_owner:
                base += 104.02032884519402
            if t.owner == self.leader_owner:
                base += 99.59965891423438
            elif t.owner not in (-1, w.player):
                base += min(42.0, self.owner_power.get(t.owner, 0.0) / 45.0)
            # Weak-target bonus (garrison discount).
            base += max(0.0, 30.0 - t.ships) * 1.0063298956721398
