...
Patched locally as V8: V7 base with tiered LEARN failure reasons, FFA aggressor
profiling, target-specific buffers, and light ROI filtering.
...
_MEMORY = {
    ...
    "target_margin": defaultdict(float), # target_id -> extra capture multiplier
...
def _aim_solution(source, target, ships, angular_velocity, comet_paths, planets, tolerance, iterations=4):
...
def _update_owner_profile(world):
    """Record new enemy launches once, with decaying 4p threat memory."""
...
def _owner_threat_scores(world):
    ...
    for owner, profile in _MEMORY["owner_profile"].items():
        if owner == world.player:
            continue
        score = (
            0.12 * profile["us_ships"] +
            3.0 * profile["us_fleets"] +
            0.65 * profile["launches"] -
            0.045 * profile["other_ships"] -
            0.85 * profile["other_fleets"]
        )
        scores[owner] = max(0.0, min(60.0, score))
    return scores


def _world_is_duel(world):
    owners = set()
...
class Context:
    __slots__ = (
        ...
        "threat_score", "nemesis_owner", "nemesis_score",
    ...
    def _build_reserves_and_surplus(self):
        ...
        for p in w.my_planets:
            ...
            nearest_nemesis = min(
                (_dist(p.x, p.y, e.x, e.y) for e in w.enemy_planets
                 if e.owner == self.nemesis_owner),
                default=999.0,
            )
            reserve = self._reserve_for(p, nearest_enemy, nearest_nemesis)
            self.reserve_by_id[p.id] = reserve
            self.surplus_by_id[p.id] = max(0, p.ships - reserve)


    def _reserve_for(self, planet, nearest_enemy, nearest_nemesis):
        ...
        # FFA nemesis buffer: if one opponent is repeatedly choosing us, keep
        # a modest border guard before spending into greedy expansion.
        if self.nemesis_owner != -1 and incoming <= 0 and step >= 45:
            if nearest_nemesis < 20.0:
                return max(5, planet.production + 4)
            if nearest_nemesis < 32.0:
                return max(4, planet.production + 2)
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            if self.ffa_behind and t.owner == self.leader_owner:
                base += 110.0
            if self.nemesis_owner != -1:
                if t.owner == self.nemesis_owner:
                    base += min(45.0, self.nemesis_score * 1.25)
                elif t.owner == -1:
                    my_dist = min((_dist(t.x, t.y, p.x, p.y) for p in w.my_planets), default=999.0)
                    nemesis_dist = min(
                        (_dist(t.x, t.y, p.x, p.y) for p in w.enemy_planets
                         if p.owner == self.nemesis_owner),
                        default=999.0,
                    )
                    if nemesis_dist < my_dist + 8.0:
                        base -= min(28.0, self.nemesis_score * 0.8)
    ...
    def capture_need(self, target, eta):
        ...
        target_mult = 1.0 if self.is_duel else 1.0 + _MEMORY["target_margin"].get(target.id, 0.0)
...
def _best_move_for_source(source, surplus, world, ctx):
    ...
    for target in world.planets:
        ...
        if target.owner == -1 and not ctx.duel_opening:
            payback_time = send / max(1, target.production)
            payback_limit = 32.0 if ctx.ffa_opening else 26.0
            if ctx.ffa_behind:
                payback_limit += 6.0
            if target.production <= 1 and payback_time > payback_limit + 10.0:
                continue
            if payback_time > payback_limit:
                score -= min(4.0, (payback_time - payback_limit) * 0.22)
...
def _apply_failure_learning(target_id, reason, turn, duel_mode=False):
    if duel_mode:
    ...
    if reason == FAIL_UNDER_SEND:
        if record[reason] >= 2:
            _MEMORY["target_margin"][target_id] = min(
                0.35, _MEMORY["target_margin"][target_id] + 0.10)
    elif reason == FAIL_CONTESTED:
        if record[reason] >= 2:
            _MEMORY["target_margin"][target_id] = min(
                0.16, _MEMORY["target_margin"][target_id] + 0.03)
    elif reason in (FAIL_ETA_MISS, FAIL_PATH_RISK):
        _MEMORY["path_risk"][target_id] += 1
        if _MEMORY["path_risk"][target_id] >= 2:
            _MEMORY["path_tolerance"] = min(0.55, _MEMORY["path_tolerance"] + 0.04)
...
def learn(world, ctx):
    ...
    for d in last:
        ...
        if target_now.owner == expected_owner:
            ...
            _MEMORY["strike_target"][target_id] += 1
            duel_mode = _world_is_duel(world)
            strikes, blacklist_turns = _apply_failure_learning(target_id, reason, turn, duel_mode)
