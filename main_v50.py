"""
Orbit Wars V50 — The God Mode Chimera.

Evolution from V49 (Dual-Brain 703.8 ELO).
Key change: FFA Brain Unchained.
Injects the Naked Exploit (+88 for empty planets) and gates ALL
high-risk exploits (Crash, Vulture, Naked, Conflict) exclusively
to the FFA Brain (not is_duel). In a Duel, it plays pure defense.
...
class Context:
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            # V50 GOD MODE: Gate all high-risk exploits behind FFA Brain.
            # In a duel, we do not want to take risky snipes that can be baited.
            if not self.is_duel:
                # V45: UNCAPPED NAKED EXPLOIT + VULTURE
                if t.owner not in (-1, w.player):
                    ...
                    if t.ships <= max(3, t.production + 1):
                        naked_bonus = 40.0 + (t.production * 16.0)
                        naked_bonus -= t.ships * 5.0
                ...
                # V19.1 UPGRADE 4: Surgical Vulture
