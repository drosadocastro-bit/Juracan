"""
Orbit Wars V51 — The Economic Snowball.

Evolution from V49 (Dual-Brain 703.8 ELO).
Key change: Hyper-aggressive opening phase. Sacrifices early ship count
by significantly boosting the heuristic value of `t.production` and relaxing
the garrison reserve ONLY when the nearest enemy is > 22.0 distance away.
This allows massive mid-game production snowballs without leaving the agent
vulnerable to early adjacent rushes.
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            if self.duel_opening and t.owner == -1:
                base += t.production * 120.0  # V51 Economic Snowball
                base += max(0.0, 18.0 - t.ships) * 2.2
            if self.ffa_opening and t.owner == -1:
                base += t.production * 65.0  # V51 Economic Snowball
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        if ctx.duel_opening and target.owner == -1:
            score = base - send * 0.9 - distance * 1.45 - eta * 2.0  # V51 relaxed send penalty
            if target.production >= 3:
                score += 90.0  # V51 snowball push
            if target.ships <= 12:
                score += 28.0
            # In duels, raw tempo matters more than pure efficiency.
            score /= math.sqrt(max(1, send))
        elif ctx.ffa_opening and target.owner == -1:
            score = base - send * 1.2 - distance * 0.72 - eta * 1.35  # V51 relaxed send penalty
            if target.production >= 3:
                score += 40.0  # V51 snowball push
            if target.ships <= 14:
                score += 14.0
