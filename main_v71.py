...
Patched locally as V7.1: deterministic micro-jitter for close target choices and
low-confidence FFA neutral timing, while preserving emergency, intercept, and
duel-opening behavior.
...
def _stable_noise(*items):
    """Deterministic pseudo-random value in [-1, 1] from game-visible fields."""
    h = 2166136261
    for item in items:
        for ch in str(item):
            h ^= ord(ch)
...
class Context:
    ...
    def _build_power_table(self):
        ...
        map_salt = sum((p.id + 3) * (p.production + 5) for p in w.planets)
        ffa_cutoff = 72 + int(round(_stable_noise("ffa_cutoff", w.player, map_salt) * 4.0))
        self.ffa_opening = (not self.is_duel) and w.step < ffa_cutoff
        self.ffa_behind = (
    ...
    def _reserve_for(self, planet, nearest_enemy):
        ...
        if self.ffa_opening and incoming <= 0:
            ...
            if step < 48:
                reserve = max(1, planet.production // 2)
                n = _stable_noise("reserve_open", w.player, step // 5, planet.id, planet.ships)
                if nearest_enemy >= 30.0 and n < -0.68:
                    reserve -= 1
                elif nearest_enemy >= 30.0 and n > 0.78:
                    reserve += 1
...
def _score_micro_jitter(world, ctx, priority, source, target, send):
    """Tiny deterministic tie-breaker; never touches hard safety priorities."""
    if priority in (PRI_EMERGENCY, PRI_INTERCEPT, PRI_DUEL_OPEN):
        return 0.0
    if ctx.duel_opening:
        return 0.0
    n = _stable_noise(
        "score_jitter", world.player, world.step // 3,
        source.id, target.id, target.owner, target.ships, send,
    )
    if ctx.is_duel:
        return n * 0.045
    if target.owner == -1:
...
def _soft_edge_skip(world, ctx, priority, source, target, send, surplus, score):
    """Occasionally wait on marginal FFA neutral moves; never on critical moves."""
    if ctx.is_duel or ctx.duel_opening or ctx.ffa_behind:
        return False
    if priority in (PRI_EMERGENCY, PRI_INTERCEPT, PRI_DUEL_OPEN, PRI_PRESSURE):
        return False
    if target.owner != -1 or world.step < 55:
        return False
    if target.production >= 4 or (target.production >= 3 and target.ships <= 14):
        return False
    n = _stable_noise("soft_wait", world.player, world.step // 2,
                      source.id, target.id, int(send), int(surplus))
    return n < -0.88


def _maybe_take_second_choice(world, ctx, source, best, best_rank, second, second_rank):
    """Use controlled variation only when two moves are strategically similar."""
    if best is None or second is None:
        return best
    if best[0] != second[0]:
        return best
    if best[0] in (PRI_EMERGENCY, PRI_INTERCEPT, PRI_DUEL_OPEN):
        return best
    if ctx.duel_opening:
        return best
    best_score = -best_rank[1]
    second_score = -second_rank[1]
    gap = best_score - second_score
    margin = 0.42 if ctx.is_duel else 1.35
    if gap < 0.0 or gap > margin:
        return best
    if ctx.is_duel and world.step < 70:
        return best
    _, _, _, best_target, _, _ = best
    _, _, _, second_target, _, _ = second
    n = _stable_noise("second_choice", world.player, world.step // 4,
                      source.id, best_target.id, second_target.id)
    threshold = 0.76 if ctx.is_duel else 0.58
    return second if n > threshold else best


def _jitter_send_size(world, ctx, priority, source, target, send, max_send):
    """Small one-way send variation; never reduces a planned capture force."""
    if priority in (PRI_EMERGENCY, PRI_INTERCEPT, PRI_DUEL_OPEN):
        return send
    if ctx.duel_opening or max_send - send < 3:
        return send
    n = _stable_noise("send_jitter", world.player, world.step // 3,
                      source.id, target.id, target.owner, int(send), int(max_send))
    threshold = 0.91 if ctx.is_duel else 0.84
    if n > 0.975 and max_send - send >= 5:
        return send + 2
    if n > threshold:
        return send + 1
    return send
...
def _best_move_for_source(source, surplus, world, ctx):
    ...
    for target in world.planets:
        ...
        score += _score_micro_jitter(world, ctx, priority, source, target, send)
        if _soft_edge_skip(world, ctx, priority, source, target, send, surplus, score):
            continue
        ...
        if rank < best_rank:
            ...
            second_rank = rank
            second = (priority, tag, source, target, int(send), angle)

    return _maybe_take_second_choice(world, ctx, source, best, best_rank, second, second_rank)
...
def act(world, ctx, decisions):
    ...
    for priority, tag, source, target, ships, angle in decisions:
        ...
        send = _jitter_send_size(world, ctx, priority, source, target, send, max_send)
