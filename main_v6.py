...
Patched locally as V6: FFA tempo mode so 4-player games expand harder without
weakening the V5 duel opening.
...
Architecture (adapted from the Julia OODA-L framework):
    ...
    DECIDE   -> priority cascade (EMERGENCY_DEFEND > INTERCEPT > DUEL_OPENING >
                FFA_TEMPO_EXPAND > SNIPE > PRESSURE_LEADER > EXPAND > STOCKPILE).
                One action per source per turn except in EMERGENCY.
...
TAG_FFA_TEMPO = "FFA_TEMPO_EXPAND"
...
class Context:
    __slots__ = (
        ...
        "is_duel", "duel_opening", "ffa_tempo", "ffa_breakout",
    ...
    def _build_power_table(self):
        ...
        self.ffa_tempo = (not self.is_duel) and w.step < 115
        self.ffa_breakout = (
            (not self.is_duel) and
            45 <= w.step < 150 and
            self.leader_owner != -1 and
            my_power < leader_power * 0.86
        )
    ...
    def _reserve_for(self, planet, nearest_enemy):
        ...
        # FFA tempo: V5 was protecting young 4-player empires too much. When no
        # fleet is already coming, spend more of the garrison on neutral growth
        # and catch-up launches, while still respecting close enemy planets.
        if (self.ffa_tempo or self.ffa_breakout) and incoming <= 0:
            if nearest_enemy < 16.0:
                return max(3, planet.production + 1)
            if nearest_enemy < 26.0 and step >= 55:
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            if self.ffa_tempo and t.owner == -1:
                base += t.production * 52.0
                base += max(0.0, 22.0 - t.ships) * 1.7
                if t.production >= 3:
                    base += 36.0
            if self.ffa_breakout:
...
def _best_move_for_source(source, surplus, world, ctx):
    ...
    for target in world.planets:
        ...
        if _is_intercept_opportunity(target, eta, ctx, world):
            ...
            tag = TAG_DUEL_OPEN
        elif (ctx.ffa_tempo or ctx.ffa_breakout) and target.owner == -1:
            priority = PRI_FFA_TEMPO
            tag = TAG_FFA_TEMPO
        ...
        if ctx.duel_opening and target.owner == -1:
            ...
            score = base - send * 1.28 - distance * 0.95 - eta * 1.65
            if target.production >= 3:
                score += 34.0
            if target.ships <= 14:
                score += 26.0
            if step < 55 and distance < 34.0:
                score += 22.0
            # 4p openings reward growth tempo more than perfect ship efficiency.
            score /= math.sqrt(max(1, send))
        else:
