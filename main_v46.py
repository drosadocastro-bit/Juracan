"""
Orbit Wars V46 — The Naked Locality Juggernaut.

Evolution from V45 (Naked Exploit, +0.233) and V43 (Locality Bonus, +0.188).
Key change: Merges the two most successful recent tuning paths.
1. Retains V45's Naked Exploit to perfectly snipe undefended planets.
2. Re-introduces V43's purely additive Locality Bonus to prefer nearby targets
   without penalizing intercontinental power projection.
...
# V46: Locality bonus — purely additive nearby preference (from V43).
# Adds a small score boost for nearby targets without touching distant scores.
# At distance 0: +LOCALITY_BONUS_MAX. At LOCALITY_RADIUS: +0. Beyond: +0.
LOCALITY_BONUS_MAX = 15.0   # max bonus at distance 0
LOCALITY_RADIUS = 80.0      # bonus tapers to 0 at this distance
...
class Context:
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            # V45: NAKED EXPLOIT & VULTURE OFFENSE
            # Fixes the "transit expiration" blind spot by scoring entirely exposed
            # planets even after their launched fleets have landed.
            if t.owner not in (-1, w.player):
                ...
                if outflow >= 6:
                    ...
                    if ratio >= 0.4:
                        v_bonus = (ratio - 0.4) * 100.0
                        v_bonus *= (1.0 + 0.25 * t.production)
                        v_bonus = min(60.0, v_bonus)
                
                naked_bonus = 0.0
                if t.ships <= max(3, t.production + 1):
                    naked_bonus = 40.0 + (t.production * 12.0)
                    naked_bonus -= t.ships * 6.0
                    naked_bonus = max(0.0, naked_bonus)
                
                # Take the max so we don't double-count the vulnerability
                base += max(v_bonus, naked_bonus)
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        # V46: Locality bonus (from V43) — purely additive nearby preference.
        # Distant targets score identically to V39; nearby targets get a small edge.
