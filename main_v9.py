...
Patched locally as V9: V7 remains the decision spine. Adds soft OODA-L advisors:
opponent threat profiling, light target-specific failure learning, and a brief
consolidation posture after frontier collapses. These advisors nudge reserves,
scores, and aim depth; they do not add V8-style hard vetoes.
...
FAIL_UNDER_SEND = "UNDER_SEND"
FAIL_CONTESTED = "CONTESTED"
FAIL_ETA_MISS = "ETA_MISS"
FAIL_PATH_RISK = "PATH_RISK"
...
def _new_failure_record():
    return defaultdict(int)


def _new_owner_profile():
    return {
        "us_ships": 0.0,
        "us_fleets": 0.0,
        "other_ships": 0.0,
...
_MEMORY = {
    ...
    "failure_reason": defaultdict(_new_failure_record),
    "target_margin": defaultdict(float),  # target_id -> small extra send multiplier
    "path_risk": defaultdict(int),        # target_id -> repeated aim/path concerns
    "seen_enemy_fleets": set(),
    "owner_profile": defaultdict(_new_owner_profile),
    "profile_last_decay": -1,
    "empire_history": [],
    "consolidate_until": -1,
}
...
def _reset_memory():
    ...
    _MEMORY["failure_reason"] = defaultdict(_new_failure_record)
    _MEMORY["target_margin"] = defaultdict(float)
    _MEMORY["path_risk"] = defaultdict(int)
    _MEMORY["seen_enemy_fleets"] = set()
    _MEMORY["owner_profile"] = defaultdict(_new_owner_profile)
    _MEMORY["profile_last_decay"] = -1
...
def _decay_owner_profile(turn):
    last = _MEMORY.get("profile_last_decay", -1)
    if last == turn:
        return
    steps = 1 if last < 0 else max(1, min(24, turn - last))
    decay = 0.985 ** steps
    for profile in _MEMORY["owner_profile"].values():
        for key in ("us_ships", "us_fleets", "other_ships", "other_fleets", "launches"):
            profile[key] *= decay
    _MEMORY["profile_last_decay"] = turn


def _update_owner_profile(world):
    """Decaying opponent profile. Used as a soft advisor, never a veto."""
    _decay_owner_profile(world.step)
    seen = _MEMORY["seen_enemy_fleets"]
    profiles = _MEMORY["owner_profile"]
    for fleet in world.fleets:
        if fleet.owner == world.player or fleet.id in seen:
            continue
        seen.add(fleet.id)
        if fleet.owner < 0:
            continue
        profile = profiles[fleet.owner]
        profile["launches"] += 1.0
        hit = world.fleet_forecasts.get(fleet.id)
        if hit is None:
            continue
        target = world.planet_by_id.get(hit[0])
        if target is None:
            continue
        if target.owner == world.player:
            profile["us_fleets"] += 1.0
            profile["us_ships"] += float(fleet.ships)
        elif target.owner >= 0 and target.owner != fleet.owner:
            profile["other_fleets"] += 1.0
            profile["other_ships"] += float(fleet.ships)


def _owner_threat_scores(world):
    scores = {}
    for owner, profile in _MEMORY["owner_profile"].items():
        if owner == world.player:
            continue
        score = (
            0.10 * profile["us_ships"] +
            2.6 * profile["us_fleets"] +
            0.55 * profile["launches"] -
            0.040 * profile["other_ships"] -
            0.75 * profile["other_fleets"]
        )
        scores[owner] = max(0.0, min(45.0, score))
    return scores


def _aim_iterations_for(target_id):
    failures = _MEMORY["failure_reason"].get(target_id, {})
    path_failures = _MEMORY["path_risk"].get(target_id, 0)
    eta_failures = failures.get(FAIL_ETA_MISS, 0) if failures else 0
    if path_failures >= 2 or eta_failures >= 2:
        return 6
    return 4


def _update_empire_stability(world):
    """Trigger a short consolidation posture after a sharp frontier loss."""
    ...
    if peak[1] >= 10 and my_planets <= peak[1] - 3:
        ...
        if until > _MEMORY.get("consolidate_until", -1):
            ...
            _log("consolidate_trigger", peak_turn=peak[0],
                 peak_planets=peak[1], current_planets=my_planets,
                 until=until)


def _enemy_fleet_near_target(world, target_id, window=3):
    for fleet in world.fleets:
        ...
        if hit is None:
            continue
        if hit[0] == target_id and hit[1] <= window:
            return True
    return False


def _classify_failure(world, decision, target_now):
    target_id = int(decision["target"])
    original_owner = int(decision.get("target_owner", -1))
    original_ships = int(decision.get("target_ships", target_now.ships))
    production = int(decision.get("target_production", target_now.production))
    planned_eta = float(decision.get("planned_eta", decision.get("eta", 1.0)))

    if target_now.owner not in (world.player, original_owner):
        return FAIL_CONTESTED
    if _enemy_fleet_near_target(world, target_id):
        return FAIL_CONTESTED

    if original_owner == -1:
        expected_without_hit = original_ships
    else:
        expected_without_hit = original_ships + int(max(0.0, planned_eta - 1.0) * production)

    if target_now.owner == original_owner:
        unchanged = abs(target_now.ships - expected_without_hit) <= max(4, production * 3)
        if unchanged and (decision.get("target_orbiting") or decision.get("target_comet")):
            return FAIL_ETA_MISS
        if unchanged:
            return FAIL_PATH_RISK
        if target_now.ships <= max(12, int(decision.get("ships", 0) * 0.5)):
            return FAIL_UNDER_SEND

    return FAIL_UNDER_SEND


def _decay_target_learning(target_id):
    if _MEMORY["strike_target"][target_id] > 0:
        _MEMORY["strike_target"][target_id] -= 1
    margin = _MEMORY["target_margin"].get(target_id, 0.0)
    if margin > 0.0:
        margin *= 0.78
        if margin < 0.015:
            _MEMORY["target_margin"].pop(target_id, None)
        else:
            _MEMORY["target_margin"][target_id] = margin
    record = _MEMORY["failure_reason"].get(target_id)
    if record:
        for key in list(record.keys()):
            record[key] = max(0, record[key] - 1)
    if _MEMORY["path_risk"].get(target_id, 0) > 0:
        _MEMORY["path_risk"][target_id] -= 1


def _apply_failure_learning(target_id, reason, turn):
    record = _MEMORY["failure_reason"][target_id]
    record[reason] += 1

    if reason == FAIL_UNDER_SEND:
        _MEMORY["target_margin"][target_id] = min(
            0.10, _MEMORY["target_margin"][target_id] + 0.035)
    elif reason == FAIL_CONTESTED:
        _MEMORY["target_margin"][target_id] = min(
            0.06, _MEMORY["target_margin"][target_id] + 0.02)
    elif reason in (FAIL_ETA_MISS, FAIL_PATH_RISK):
        _MEMORY["path_risk"][target_id] += 1
        if _MEMORY["path_risk"][target_id] >= 2:
            _MEMORY["path_tolerance"] = min(0.45, _MEMORY["path_tolerance"] + 0.025)

    strikes = _MEMORY["strike_target"][target_id]
    blacklist_turns = 0
    if strikes >= 3:
        blacklist_turns = 25
        _MEMORY["blacklist"][target_id] = turn + blacklist_turns
        _MEMORY["strike_target"][target_id] = 0
        _MEMORY["capture_buffer_mult"] = min(1.5, _MEMORY["capture_buffer_mult"] + 0.06)
        _MEMORY["path_tolerance"] = min(0.50, _MEMORY["path_tolerance"] + 0.04)
        for key in list(record.keys()):
            record[key] = max(0, record[key] // 2)
    return strikes, blacklist_turns
...
class Context:
    __slots__ = (
        ...
        "threat_score", "nemesis_owner", "nemesis_score", "consolidating",
    ...
    def _build_power_table(self):
        ...
        self.threat_score = _owner_threat_scores(w)
        self.nemesis_owner = -1
        self.nemesis_score = 0.0
        if self.threat_score and not self.is_duel:
            self.nemesis_owner = max(self.threat_score, key=lambda o: self.threat_score[o])
            self.nemesis_score = self.threat_score.get(self.nemesis_owner, 0.0)
            if self.nemesis_score < 8.0:
                self.nemesis_owner = -1
                self.nemesis_score = 0.0
        self.consolidating = _MEMORY.get("consolidate_until", -1) > w.step
    ...
    def _build_reserves_and_surplus(self):
        ...
        for p in w.my_planets:
            nearest_enemy = 999.0
            nearest_enemy_owner = -1
            for e in w.enemy_planets:
                d = _dist(p.x, p.y, e.x, e.y)
                if d < nearest_enemy:
                    nearest_enemy = d
                    nearest_enemy_owner = e.owner
            reserve = self._reserve_for(p, nearest_enemy, nearest_enemy_owner)
            self.reserve_by_id[p.id] = reserve
            self.surplus_by_id[p.id] = max(0, p.ships - reserve)


    def _reserve_for(self, planet, nearest_enemy, nearest_enemy_owner=-1):
        ...
        soft_floor = 0
        if incoming <= 0:
            if self.consolidating and step >= 55:
                if nearest_enemy < 34.0:
                    soft_floor = max(soft_floor, planet.production * 2 + 4)
                else:
                    soft_floor = max(soft_floor, planet.production + 2)
            if (not self.is_duel and nearest_enemy_owner == self.nemesis_owner and
                    self.nemesis_score >= 12.0 and nearest_enemy < 34.0):
                soft_floor = max(soft_floor, planet.production + 3)
        ...
        if self.duel_opening and incoming <= 0:
            if step < 22:
                return 1
            return max(1, planet.production // 2, soft_floor)
        ...
        if self.ffa_opening and incoming <= 0:
            if nearest_enemy < 18.0:
                return max(3, planet.production + 1, soft_floor)
            if step < 48:
                return max(1, planet.production // 2, soft_floor)
            return max(2, planet.production, soft_floor)
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            if (not self.is_duel) and t.owner == self.nemesis_owner:
                base += min(45.0, self.nemesis_score * 0.9)
            if self.consolidating:
                if t.owner == -1:
                    base -= 18.0 + max(0.0, t.ships - 10.0) * 0.6
                elif t.owner not in (-1, w.player):
                    base += 16.0
            failures = _MEMORY["failure_reason"].get(t.id)
            if failures:
                base -= min(24.0, 4.5 * sum(failures.values()))
            path_risk = _MEMORY["path_risk"].get(t.id, 0)
            if path_risk:
                base -= min(16.0, 5.0 * path_risk)
            # Weak-target bonus (garrison discount).
            base += max(0.0, 30.0 - t.ships) * 0.9
            # Comet penalty: avoid spending ships on planets about to leave.
    ...
    def capture_need(self, target, eta):
        ...
        target_mult = 1.0
        if not self.is_duel:
            target_mult += min(0.10, _MEMORY["target_margin"].get(target.id, 0.0))
        return int(math.ceil(remaining * _MEMORY["capture_buffer_mult"] * target_mult))
...
def _best_move_for_source(source, surplus, world, ctx):
    ...
    for target in world.planets:
        ...
        iterations = _aim_iterations_for(target.id)
        aim = _aim_solution(source, target, probe,
                            world.angular_velocity, world.comet_paths,
                            world.planets, _MEMORY["path_tolerance"], iterations)
        if aim is None:
            continue
        angle, eta, _, _ = aim
        ...
        if target.owner == -1 and not ctx.duel_opening:
            payback_time = send / max(1.0, float(target.production))
            if payback_time > 22.0:
                score -= min(6.0, (payback_time - 22.0) * 0.25)
...
def act(world, ctx, decisions):
    ...
    for priority, tag, source, target, ships, angle in decisions:
        ...
        tagged.append({
            ...
            "ships": int(send),
            "eta": float(eta),
            "planned_eta": float(eta),
            "target_owner": int(target.owner),
            "target_ships": int(target.ships),
            "target_production": int(target.production),
            "target_orbiting": bool(_is_orbiting(target)),
            "target_comet": bool(target.id in world.comet_paths),
        })
...
def learn(world, ctx):
    ...
    for d in last:
        ...
        if target_now.owner == expected_owner:
            # Success: let target-specific caution decay quickly.
            _decay_target_learning(target_id)
            ...
            reason = _classify_failure(world, d, target_now)
            _MEMORY["strike_target"][target_id] += 1
            strikes, blacklist_turns = _apply_failure_learning(target_id, reason, turn)
            _log("learn_miss", target=target_id, tag=d["tag"], reason=reason,
                 strikes=strikes, expected_owner=expected_owner,
                 actual_owner=int(target_now.owner),
                 target_margin=_MEMORY["target_margin"].get(target_id, 0.0),
                 path_risk=_MEMORY["path_risk"].get(target_id, 0))

            # 3-STRIKE RULE: still V7's escape hatch, with lighter padding.
            if blacklist_turns:
                ...
                _log("learn_rewrite", target=target_id,
                     reason=reason,
                     blacklist_until=turn + blacklist_turns,
