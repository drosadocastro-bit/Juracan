"""
Orbit Wars V52 — The Blitzkrieg.

Evolution from V49 (Dual-Brain 703.8 ELO).
Key change: Duel-Only All-In Rush.
During the duel opening phase, the agent abandons neutral expansion entirely.
It drops its home reserve to 0 and assigns a massive +500 priority to enemy
planets, launching its entire starting fleet at the opponent to end the game
in the first 40 turns. If the rush fails, the game is lost.
...
LOCALITY_RADIUS = 30.0    # V43: Max distance for locality bonus
LOCALITY_BONUS_MAX = 10.0 # V43: Max additive bonus for immediate neighbors
...
class Context:
    ...
    def _reserve_for(self, planet, nearest_enemy):
        ...
        # V52 BLITZKRIEG: 0 reserves during duel opening to full-send immediately.
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            elif self.duel_opening and t.owner == self.leader_owner:
                base += 500.0  # V52 Blitzkrieg priority override
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        if vulture_mode:
            ...
            tag = TAG_INTERCEPT
        elif ctx.duel_opening and (target.owner == -1 or target.owner == ctx.leader_owner):
        ...
        elif ctx.duel_opening and target.owner == ctx.leader_owner:
            # V52 BLITZKRIEG: Favor hitting the enemy planet incredibly fast
            score = base - distance * 0.5 - eta * 1.0
            score /= math.sqrt(max(1, send))
