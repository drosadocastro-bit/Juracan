"""
Orbit Wars V47 — The Tactical Chimera (Frankenstein A).

Evolution from V39 (gold baseline, 729.9 ELO).
Key change: Stitches together our best offensive micro-tactics.
Uses V39's Calibrated Fleet Doctrine & Crash Exploit, combined with
a tuned-down version of V45's Naked Exploit (capped at +40) to snipe
undefended planets without causing the agent to over-commit.
...
class Context:
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            # V47: UNCAPPED NAKED EXPLOIT (Chimera Limb 2)
            # Fixes the transit expiration blind spot. Fully unchained!
            if t.owner not in (-1, w.player):
                ...
                if outflow >= 6:
                    ...
                    if ratio >= 0.4:
                        v_bonus = (ratio - 0.4) * 100.0  # Restored to 100.0
                        v_bonus *= (1.0 + 0.25 * t.production)  # Restored to 0.25
                        v_bonus = min(60.0, v_bonus)  # Restored cap to 60
                
                naked_bonus = 0.0
                if t.ships <= max(3, t.production + 1):
                    naked_bonus = 40.0 + (t.production * 16.0)  # Restored to max ~88
