"""
Orbit Wars V42 — The Intercontinental Juggernaut.

Evolution from V39 (gold baseline, 729.9 ELO).
Key change: Vulture/Crash/Elimination bonuses are moved from the global
_score_targets() into _best_move_for_source() and scaled by a soft spatial
discount. This avoids V41's "Isolationism Trap" (hard gates caused turtling)
while gently preferring closer targets at equal value.

Soft discount formula:
    spatial_factor = max(0.15, 1.0 - (distance / 160.0) * 0.85)
    distance  0 → 1.00 | distance 50 → 0.73 | distance 80 → 0.55 | diagonal → 0.25
...
# V42: Soft spatial discount for opportunity bonuses (Vulture/Crash/Elimination).
# Applied inside _best_move_for_source() so bonuses scale smoothly with distance
# instead of hard cut-offs. Floor at 0.15 preserves intercontinental reach.
SPATIAL_DISCOUNT_SCALE = 160.0  # reference distance (board diagonal ≈ 141)
SPATIAL_DISCOUNT_SLOPE = 0.85   # how fast discount ramps from 1.0 to floor
SPATIAL_DISCOUNT_FLOOR = 0.15   # minimum factor — never fully zero
    ...
    def _score_targets(self):
        ...
        for t in w.planets:
            ...
            if t.owner == self.leader_owner:
                base += 72.0
            # V42: Elimination bonus (160.0) moved to _best_move_for_source with soft discount.
            ...
            # V42: Vulture, Crash, and Elimination bonuses are moved to
            # _best_move_for_source() where they're scaled by soft spatial discount.
            # Only the conflict bonus stays here since it's distance-independent.
...
def _best_move_for_source(source, surplus, world, ctx, excluded_targets=None):
    ...
    for target in world.planets:
        ...
        # V42: Soft spatial discount for opportunity bonuses.
        # Preserves intercontinental reach (floor = 0.15) while gently
        # preferring closer targets. Replaces V41's hard spatial gates.
        _sf = max(SPATIAL_DISCOUNT_FLOOR,
                  1.0 - (distance / SPATIAL_DISCOUNT_SCALE) * SPATIAL_DISCOUNT_SLOPE)

        # V42: Vulture Offense (was global in V39 _score_targets; now distance-scaled).
        if target.owner not in (-1, world.player):
            ...
            if outflow >= 6:
                ...
                if ratio >= 0.4:
                    ...
                    base += min(60.0, vulture_bonus) * _sf

        # V42: Crash Exploit (was global in V39 _score_targets; now distance-scaled).
        crash = ctx.crash_bonus.get(target.id, 0.0)
        if crash > 0.0:
            base += min(80.0, crash) * _sf

        # V42: Elimination Drive (was +160.0 global in V39; now distance-scaled).
